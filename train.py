...
tokens = torch.tensor(tokens, dtype=torch.long).unsqueeze(0).repeat(5, 1)  # (5, 8)
tokens = tokens.to(device)  # Move tokens to the device
...
for i in range(5):
    decoded = enc.decode(tokens[i, :30].tolist())
    print(">", decoded)
